...
class SaverPlyPlugin : public sd::frontend::Saver {
...
public:
  ...
  virtual SaverPlyPlugin*
  clone() {
    return new SaverPlyPlugin;
  }

  virtual const std::string&
  formatName() const {
    static const std::string fName = "Ply format";
    return fName;
  }

  virtual const std::string&
  fileExtensionFilters() const {
    static const std::string ext = "*.ply;";
    return ext;
  }
  ...
  save() {
    ...
    sd::ply::SaverPly<T, N> saver;
    bool successful = saver.savePlyImage(mesh, filename,
...
template<typename T, unsigned int N> const std::string SaverPlyPlugin<T, N>::m_name = "SaverPlyPlugin";
template<typename T, unsigned int N> const sd::frontend::AlgorithmInfo SaverPlyPlugin<T, N>::m_input = sd::frontend::make_info((sd::core::MeshView*) NULL);
template<typename T, unsigned int N> const sd::frontend::ParameterList SaverPlyPlugin<T, N>::m_parameters = sd::frontend::make_parameter_list("filename", "", "VertexNormals;VertexColors;FaceNormals;FaceColors", true);
...
void
registerPlugin() {
  sd::frontend::registerSaver(new SaverPlyPlugin<sd::DOUBLE, 3>);
  sd::frontend::registerSaver(new SaverPlyPlugin<sd::DOUBLE, 4>);
  sd::frontend::registerSaver(new SaverPlyPlugin<sd::FLOAT, 3>);
  sd::frontend::registerSaver(new SaverPlyPlugin<sd::FLOAT, 4>);
}
