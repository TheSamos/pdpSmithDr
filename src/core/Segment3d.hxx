...
#ifndef SD_CORE_SEGMENT3D_HXX
#define SD_CORE_SEGMENT3D_HXX
/*
template<typename T>
bool
sd::core::Segment3d<T>::isCoplanar(const Segment3d<T>& line) const {
...
template<typename T>
inline bool
sd::core::Segment3d<T>::isSame(const sd::core::Segment3d<T>& line) const {
  return (m_point1 == line.m_point1 && m_point2 == line.m_point2)
    || (m_point1 == line.m_point2 && m_point2 == line.m_point1);
}

template<typename T>
bool
sd::core::Segment3d<T>::contains(const sd::core::Point3d<T>& point) const {
...
template<typename T>
sd::core::Point3d<T>
sd::core::Segment3d<T>::intersection(const Segment3d<T>& line) const {
  const Point3d<T>& p = m_point;
  const Point3d<T>& q = line.m_point;
  const Vector3d<T>& v1 = m_direction;
  const Vector3d<T>& v2 = line.m_direction;
...
template<typename T>
sd::core::Point3d<T>
sd::core::Segment3d<T>::orthogonalProjection(const Point3d<T>& point) const {
  if (contains(point))
    return point;
  ...
  return (*this).intersection(Segment3d<T>(point, orthogonal));
}
*/

template<typename T>
T
sd::core::Segment3d<T>::norm(const Point3d<T>& point) const {
  return Vector3d<T>(m_point1, m_point2).norm();
}

template<typename T>
sd::core::Segment3d<T>&
sd::core::Segment3d<T>::operator+=(const sd::core::Vector3d<T>& vector) {
  m_point1 += vector;
  m_point2 += vector;
}
