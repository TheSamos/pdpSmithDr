...
void
AlgorithmAction::setXMLParameterized(bool xml_parameterized)
{
    m_xml_parameterized = xml_parameterized;
}
...
AlgorithmAction::configurationComplete()
{
    return std::count_if(m_inputViews.begin(), m_inputViews.end(),
                         [&](const frontend::ViewInfoPtr & view)
    {
        return view;
    }
...
AlgorithmAction::process(bool preview)
{
    ...
    //std::vector<frontend::ParameterList> parameters;
    std::map<std::string, std::string> xml_parameters;
